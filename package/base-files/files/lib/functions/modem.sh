...
setup_ip_passthrough() {
  ...
  local wan_table=${bridge_table:-$(ip2table ${wan_ipaddr})}
  ...
  if [ "${bridge_type}" = "passthrough" ]; then
    # Required to access internet on the device itself. Might be best to skip this if multiple modems
    iptables -w -tnat -I POSTROUTING -o ${wan_device} -j SNAT --to ${wan_ipaddr}
    ip route add default dev ${wan_device}
  fi

  echo "dhcp-range=tag:${wan_device}_bridge,${wan_ipaddr},static,${wan_subnetmask},2m" > /tmp/dnsmasq.d/bridge${wan_device}
  echo "shared-network=${bridge_device},${wan_ipaddr}" >> /tmp/dnsmasq.d/bridge${wan_device}
  echo "dhcp-host=*:*:*:*:*:*,set:${wan_device}_bridge,${wan_ipaddr}" >> /tmp/dnsmasq.d/bridge${wan_device}
  echo "dhcp-option=tag:${wan_device}_bridge,${bridge_device},3,${wan_gateway}" >> /tmp/dnsmasq.d/bridge${wan_device}

  if [ -n "$wan_dns1" ] || [ -n "$wan_dns2" ]; then
    echo "dhcp-option=tag:${wan_device}_bridge,${bridge_device},6${wan_dns1:+,$wan_dns1}${wan_dns2:+,$wan_dns2}" >> /tmp/dnsmasq.d/bridge${wan_device}
    echo "server=$wan_dns1" >> /tmp/dnsmasq.d/bridge${wan_device}
    echo "server=$wan_dns2" >> /tmp/dnsmasq.d/bridge${wan_device}
  fi
...
teardown_ip_passthrough() {
  ...
  ip route del default dev ${bridge_device} table ${bridge_table}
  ip rule del iif ${bridge_device} lookup ${bridge_table}

  ip route add default dev ${wan_device} table ${wan_table}
  ip rule add iif ${bridge_device} lookup ${wan_table} pref 10000
  sysctl net.ipv4.conf.${bridge_device}.proxy_arp=1

  handle=$(nft -a list chain inet fw4 forward | grep "iifname \"${wan_device}\" oifname \"${passthrough_dev}\"" | awk -F' ' '{ print $NF }')
  nft delete rule inet fw4 forward handle ${handle}
  handle=$(nft -a list chain inet fw4 forward | grep "iifname \"${passthrough_dev}\" oifname \"${wan_device}\"" | awk -F' ' '{ print $NF }')
  nft delete rule inet fw4 forward handle ${handle}

  handle=$(nft -a list chain inet fw4 input | grep "iifname \"${wan_device}\" accept" | awk -F' ' '{ print $NF }')
  nft delete rule inet fw4 forward handle ${handle}
  handle=$(nft -a list chain inet fw4 input | grep "iifname \"${passthrough_dev}\" accept" | awk -F' ' '{ print $NF }')
  nft delete rule inet fw4 forward handle ${handle}

  # POSTROUTING is not overriden by the nft chain so lets keep using iptables
  iptables -tnat -I POSTROUTING -o ${wan_device} -j ACCEPT
  ...
  if [ "${bridge_type}" = "passthrough" ]; then
    ...
    ip route delete default dev ${wan_device}
  fi
